...
Sphere* Sphere::GenHeightMap() {
	Sphere* s = new Sphere(15.0, 16, false);
	...
	for (int z = 0; z <= phi + 1; z++) {
		for (int x = 0; x <= theta; x++) {
			...
			case 300: case 307: case 311: case 313: case 324: case 326: case 335: hei += 0.1; break;
			...
			case 290: case 295: case 304: case 308: case 316: case 330: hei -= 0.1; break;
...
Sphere* Sphere::GenWaterWave(float degree, int texRepeat) {
	Sphere* s = new Sphere(degree, texRepeat, false);
	int theta = 180 / degree;
	int phi = 360 / degree;
...
void Sphere::Update(float totalTime) {
	...
	for (int z = 2; z < phi - 1; z++) {
		for (int x = 2; x < theta - 1; x++) {
			...
			switch ((z + x) % 4) {
			case 0: polorCoord.x += 0.0005 * sin(totalTime + PI / 2);		break;
			case 1: polorCoord.x += 0.0005 * sin(totalTime + PI);			break;
			case 2: polorCoord.x -= 0.0005 * sin(totalTime + PI * 3 / 2);	break;
			case 3: polorCoord.x -= 0.0005 * sin(totalTime);				break;
			default: break;
			}
